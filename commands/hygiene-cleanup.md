...
The plugin caches the internal token from the successful plan and injects it only when the same session, repo, and options are applied with `confirmDelete: true`. Low-level direct calls still require the matching token, and stale fingerprint checks remain enforced.
