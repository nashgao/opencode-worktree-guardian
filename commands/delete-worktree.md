...
If a local Guardian branch remains after its worktree and active state are gone, pass the exact `branch` or the terminal deleted/abandoned `sessionId` with `deleteBranch: true` to request stale-branch cleanup. Guardian only plans this when terminal Guardian state or matching `refs/opencode-guardian` safety refs prove ownership, the branch exists locally, and it is checked out nowhere. Branch prefixes alone are not ownership proof.
