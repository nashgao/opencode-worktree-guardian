...
- `guardian done` -> run `node --import tsx <adapter-path> tool guardian_done '{"mode":"plan"}'` first. If the active session is dirty, include an explicit `commitMessage`; Guardian will not invent one. After explicit user confirmation, rerun with `{"mode":"apply","confirm":true}` plus the same plan options. Active-session apply commits dirty work when needed, lands the PR, proves the commit reached the remote base branch, then removes the stale worktree and local branch. Add `allowAdminBypass:true` only when the user explicitly approves a branch-protection bypass; do not copy or ask for the internal confirm token.
