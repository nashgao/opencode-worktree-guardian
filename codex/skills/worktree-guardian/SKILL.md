...
- `guardian done` -> run `node <adapter-path> tool guardian_done '{"mode":"plan"}'` first. From a clean primary base branch, bare `guardian_done` plans the repo-wide done-all lane; if the user invoked this completion workflow and the plan is safe, rerun with `{"mode":"apply","confirm":true}` plus the same plan options. Active-session apply commits dirty work when needed, lands the PR with PR head branch deletion requested, proves the commit reached the remote base branch, then removes the stale worktree and local branch. `guardian_done` may report redundant cleanup candidates, but applying that sweep requires separate `guardian_finish_workflow` confirmation. Add `allowAdminBypass:true` only when the user explicitly approves a branch-protection bypass; do not copy or ask for the internal confirm token.
