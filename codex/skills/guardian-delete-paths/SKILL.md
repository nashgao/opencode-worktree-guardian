...
Configured protected regular files require one absolute external `archivePath` and its exact lowercase `archiveSha256`. The plan must show a matching archive proof for every selected file. This never authorizes Git metadata, dependencies, worktree roots, symlink roots, or protected directories.
