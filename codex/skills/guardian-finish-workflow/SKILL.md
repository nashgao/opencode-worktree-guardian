...
After a successful apply, relay the attached `postCompletionHygiene` inventory before declaring completion. `scan-failed` or `incomplete` means the cleanup mutation may have succeeded, but the completion result remains partial until a fresh read-only scan succeeds.
