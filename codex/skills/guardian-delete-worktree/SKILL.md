...
For an evidence-only dirty worktree, use one absolute external `archivePath` plus its exact lowercase `archiveSha256`; include `allowIgnoredFiles: true` when ignored paths are present. Proceed only when the plan proves every untracked and consented ignored path by full permission mode, size, digest, or symlink target and reports the archive outside the target worktree. Tracked or staged changes and non-ordinary Git statuses remain blocked or use their existing proof lane; duplicate members, hardlinks, and special archive types block.
