...
const COMMANDS = [
  {
    ...
    prompt: "Use the guardian_done native tool. Run mode=plan first. From a clean primary branch, bare guardian_done plans the repo-wide done-all lane: after confirmation it lands finishable sessions, syncs local base, and reports redundant merged worktree/branch cleanup candidates for separate confirmation. For an active session, apply only with confirm=true; if the session is dirty, include commitMessage so Guardian can commit before creating or reusing the PR, merging it, proving remote-base reachability, removing the stale worktree/local branch, syncing local base, and reporting redundant cleanup candidates for separate confirmation. If it selects dirty primary-main publishing, require commitMessage and confirm=true; after publish, inspect the returned cleanupPlan and do not silently apply cleanup. Admin bypass requires allowAdminBypass=true. Never force-push, mutate stashes, delete remote branches, or run raw cleanup commands.",
  },
  {
