...
const COMMANDS = [
  ...
  },
  {
    ...
    prompt: "Use the guardian_goal native tool. Run mode=plan first, inspect the configured goal flags, hygiene and done child steps, blockers, selected session worktree, complete, and hygienePostcondition, then continue to mode=apply confirm=true with the same options when the plan is safe and the user invoked the goal workflow. Strict actionable plans can be planned-partial with complete=null. If metadata says freshPlanRequired, stop and obtain the fresh plan rather than applying a prior one. Include commitMessage when dirty implementation work needs to be committed. Pass exact current regular untracked source files through the same intentionalPaths list in plan and apply; the list is one-shot and token-bound. Guardian goal composes token-bound known-cleanable and filesystem-verified empty-directory hygiene cleanup before guardian_done so generated residue is not committed, then lands, pushes, and cleans through existing Guardian gates. After apply, do not treat ok as desired-state completion: inspect complete and hygienePostcondition. no-unprotected-findings can return ok=true, complete=false, status=partial after the post-apply hygiene rescan when residual findings remain; in that mode, reviewableCandidates are inventory rather than strict failures. no-unprotected-residue additionally blocks unresolved reviewableCandidates and incomplete inventory coverage. Strict mode does not broaden deletion. Every mode auto-deletes only token-bound known-cleanable findings and filesystem-verified empty directories; residual nested-git and suspicious findings require direct explicit review, dirty nested Git requires allowDirtyNestedGit, and protectedPaths deny deletion without proving retained content is useful. Inspect the bounded protected inventory marked not-assessed and cleanup-authorized=false. Resolve reviewables through one-shot intentionalPaths, intentional retention policy, or exact guardian_delete_paths planning. Do not infer garbage from extensions or use raw git cleanup, raw worktree removal, raw branch deletion, stash mutation, force-push, or bypass Guardian blockers.",
  },
  {
