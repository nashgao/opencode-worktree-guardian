...
const COMMANDS = [
  ...
  },
  {
    ...
    prompt: "Use the guardian_goal native tool. Run mode=plan first, inspect the configured goal flags, hygiene and done child steps, blockers, selected session worktree, tracked baseline, complete, and hygienePostcondition, then continue to mode=apply confirm=true with the same options when the plan is safe and the user invoked the goal workflow. Strict actionable plans can be planned-partial with complete=null. If metadata says freshPlanRequired, stop and obtain the fresh plan rather than applying a prior one. Include commitMessage when dirty implementation work needs to be committed. Pass exact current regular newly added source files through the same intentionalPaths list in plan and apply; the list is one-shot and token-bound. Guardian goal composes token-bound known-cleanable and filesystem-verified empty-directory hygiene cleanup, then compares the selected session index with its recorded start commit before guardian_done so unacknowledged tracked additions cannot be committed or pushed. After apply, do not treat ok as desired-state completion: inspect complete and hygienePostcondition. no-unprotected-findings can return ok=true, complete=false, status=partial after the post-apply hygiene rescan when residual findings remain; in that mode, reviewableCandidates are inventory rather than strict failures. no-unprotected-residue additionally blocks unresolved reviewableCandidates and incomplete inventory coverage. Strict mode does not broaden deletion. Every mode auto-deletes only token-bound known-cleanable findings and filesystem-verified empty directories; residual nested-git and suspicious findings require direct explicit review, dirty nested Git requires allowDirtyNestedGit, and protectedPaths deny deletion without proving retained content is useful or acknowledging tracked additions. Inspect the bounded protected inventory marked not-assessed and cleanup-authorized=false. Resolve reviewables through one-shot intentionalPaths, intentional retention policy, or exact guardian_delete_paths planning with allowTracked=true for tracked additions. Do not infer garbage from extensions or use raw git cleanup, raw worktree removal, raw branch deletion, stash mutation, force-push, or bypass Guardian blockers.",
  },
  {
  ...
  },
  {
    ...
    prompt: "Use the guardian_hygiene native tool. With no mode it scans only. Relay the bounded protected inventory as not-assessed with cleanup-authorized=false; protection denies deletion but does not prove retained content is useful. reviewableCandidates are inventory, not hygiene targets. For intentional reviewable cleanup, use guardian_delete_paths mode=plan paths=[...]; directories also require allowRecursive=true. Review target status and blockers before explicit confirmation, and do not pass reviewables back to guardian_hygiene. For hygiene findings, run mode=plan first, inspect exact approved targets and blockers, get explicit user confirmation, then apply with confirmDelete=true. Never run raw cleanup commands.",
  },
  {
