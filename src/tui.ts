...
const COMMANDS = [
  {
    ...
    prompt: "Use the guardian_done native tool. Run mode=plan first, inspect the selected lane, preflight, dirty files, and blockers, then continue to mode=apply confirm=true with the same options when the plan is safe and the user invoked the completion workflow. From a clean primary branch, bare guardian_done plans the repo-wide done-all lane; apply lands finishable sessions, syncs local base from its tracked upstream when that upstream remote is trusted, and reports redundant merged worktree/branch cleanup candidates for separate confirmation. For an active session, include commitMessage when the session is dirty so Guardian can commit before creating or reusing the PR, merging it without GitHub CLI branch deletion, proving remote-base reachability, removing the stale worktree/local branch, syncing local base, and reporting redundant cleanup candidates for separate confirmation. If it selects dirty primary-main publishing, require commitMessage and confirm=true; after publish, inspect the returned cleanupPlan and do not silently apply cleanup. Admin bypass requires allowAdminBypass=true. Never force-push, mutate stashes, delete unrelated remote branches, or run raw cleanup commands.",
  },
  {
