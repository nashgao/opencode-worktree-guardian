...
export function createTools(planCache: PlanTokenCache) {
  return {
    ...
    guardian_goal: guardianTool("guardian_goal", "Plan or apply config.goal through Guardian gates. Plans report complete=null; strict actionable plans may be planned-partial. After apply inspect complete and hygienePostcondition: ok=true/status=partial/complete=false is possible. no-unprotected-residue also blocks reviewables or incomplete inventory without broadening cleanup beyond token-bound known-cleanable findings. Protected roots remain non-deletable and are shown as bounded, authorization-neutral, not-assessed inventory.", planCache),
