...
export function createTools(planCache: PlanTokenCache) {
  return {
    ...
    guardian_goal: guardianTool("guardian_goal", "Plan or apply config.goal through Guardian gates. Plans report complete=null; strict actionable plans may be planned-partial. After apply inspect complete and hygienePostcondition: ok=true/status=partial/complete=false is possible. The strict default cleans token-bound known-cleanable findings and verified empty directories, rescans the selected session worktree, and blocks before commit or push on unacknowledged residue. Pass current regular untracked files through exact one-shot intentionalPaths in both phases. No mode broadens deletion authority; protected roots remain non-deletable authorization-neutral inventory.", planCache),
    ...
    guardian_hygiene: guardianTool("guardian_hygiene", "Scan bounded protected inventory as not-assessed/no-cleanup evidence, or plan/apply token-gated cleanup only for authorized workspace hygiene findings.", planCache),
