...
export function createTools(planCache: PlanTokenCache) {
  return {
    ...
    guardian_delete_paths: guardianTool("guardian_delete_paths", "Plan or apply exact path deletion with confirm-token, fingerprint, tracked-file, recursive, protected-root, and optional external archive recovery gates.", planCache),
    ...
    guardian_delete_worktree: guardianTool("guardian_delete_worktree", "Plan or apply safe Guardian-mediated worktree deletion with confirm-token, safety-ref, and optional evidence-archive gates.", planCache),
    ...
    guardian_goal: guardianTool("guardian_goal", "Plan or apply config.goal through Guardian gates. Plans report complete=null; strict actionable plans may be planned-partial. After apply inspect complete and hygienePostcondition: ok=true/status=partial/complete=false is possible. The strict default cleans token-bound known-cleanable findings and verified empty directories, then compares the selected session's index with its recorded start commit and blocks before commit or push on unacknowledged additions. Pass exact current regular newly added files through one-shot intentionalPaths in both phases. No mode broadens deletion authority; protected roots remain non-deletable but do not acknowledge tracked additions.", planCache),
    ...
    guardian_quarantine: guardianTool("guardian_quarantine", "Plan or explicitly confirm restoration or purge of one recoverable quarantine item.", planCache),
    ...
    guardian_hygiene: guardianTool("guardian_hygiene", "Scan bounded protected inventory plus newly added tracked paths relative to the selected session baseline, or plan/apply token-gated cleanup only for authorized workspace hygiene findings.", planCache),
