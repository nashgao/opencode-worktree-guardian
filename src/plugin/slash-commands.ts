...
export function rewriteGuardianCommand(input: GuardianToolInput = {}, output: GuardianToolInput = {}) {
  ...
  const match = command.trim().match(/^\/?guardian\s+(done|status|project-status|finish-workflow|finish|preserve|recover|report|start|hygiene|init|gc|delete-paths|delete-worktree|unblock-finish)(?:\s+(.*))?$/);
