...
export function rewriteGuardianCommand(input: GuardianToolInput = {}, output: GuardianToolInput = {}) {
  ...
  const goalGuidance = action === "goal" ? " Run guardian_goal with mode=plan first, inspect configured goal flags, hygiene and done child steps, blockers, and selected dirty target, then continue to mode=apply confirm=true with the same options when the plan is safe and the user invoked the goal workflow. Include commitMessage when dirty implementation work needs to be committed." : "";
