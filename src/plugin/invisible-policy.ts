...
export function buildInvisiblePolicy(config: GuardianConfig) {
  return [
    ...
    "- goal.hygieneCompletion defaults to no-unprotected-residue. Guardian cleans only token-bound known-cleanable findings and filesystem-verified empty directories, rescans the selected session worktree, and blocks before done on residual findings, unacknowledged reviewables, or incomplete coverage. Pass exact current regular untracked files through one-shot token-bound intentionalPaths in both phases. Explicit legacy modes remain available; no mode broadens deletion.",
