...
export function buildInvisiblePolicy(config: GuardianConfig) {
  return [
    ...
    "- goal.hygieneCompletion defaults to authorized-cleanup. Strict modes do not broaden deletion: every mode auto-deletes only token-bound known-cleanable findings. no-unprotected-residue additionally makes unresolved reviewableCandidates or incomplete inventory coverage completion failures. Residual nested-git, suspicious findings, and reviewable cleanup require direct explicit review. protectedPaths block deletion but do not prove retained content is useful; inspect the bounded protected inventory.",
