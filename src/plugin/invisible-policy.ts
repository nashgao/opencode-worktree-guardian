...
export function buildInvisiblePolicy(config: GuardianConfig) {
  return [
    ...
    "- Use guardian_goal for configured desired-state completion; it composes token-bound known-cleanable hygiene cleanup and guardian_done through existing gates.",
    "- After guardian_goal apply, inspect complete and hygienePostcondition. ok confirms authorized work ran, not that the desired state is complete. Strict no-unprotected-findings can return ok=true, complete=false, status=partial after its post-apply hygiene rescan.",
    "- goal.hygieneCompletion defaults to authorized-cleanup. Strict mode does not broaden deletion: both modes auto-delete only token-bound known-cleanable findings. Residual nested-git and suspicious findings require direct explicit review; dirty nested Git also requires allowDirtyNestedGit. protectedPaths are intentional retention, and reviewableCandidates are inventory rather than strict failures.",
