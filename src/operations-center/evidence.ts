...
function hygiene(input: unknown) {
  ...
  const metrics = [["Candidate Paths", number(summary.candidateCount)], ["Findings", count], ["Reviewable", number(summary.reviewableCandidateCount, candidates.length)], ["Protected Roots", number(summary.protectedInventoryCount, protectedEntries.length)], ["Protected Files", number(summary.protectedInventoryFileCount)], ["Protected Bytes", number(summary.protectedInventoryTotalBytes)], ["Manual Review", number(bySeverity.fail)], ["Warn", number(bySeverity.warn)], ["Known Cleanable", number(byCategory["known-cleanable"])], ["Nested Git", number(byCategory["nested-git"])], ["Suspicious", number(byCategory.suspicious)]];
  ...
  const protectedRows = protectedEntries.map((entry) => { const inventory = record(entry); return `<tr><td data-label="Path"><code>${escape(inventory.path)}</code></td><td data-label="Files">${number(inventory.fileCount)}</td><td data-label="Directories">${number(inventory.directoryCount)}</td><td data-label="Bytes">${number(inventory.bytes)}${inventory.bytesTruncated === true ? "+" : ""}</td><td data-label="Assessment">not-assessed</td><td data-label="Reason">${escape(inventory.reason)}</td></tr>`; }).join("") || empty(6, "No protected inventory recorded.");
