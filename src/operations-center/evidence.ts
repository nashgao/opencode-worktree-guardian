...
function hygiene(input: unknown) {
  const value = record(input); const summary = record(value.summary); const findings = array(value.findings); const candidates = array(value.reviewableCandidates); const protectedEntries = array(value.exclusions); const bySeverity = record(summary.bySeverity); const byCategory = record(summary.byCategory); const count = number(summary.findingCount, findings.length);
  const rootsTruncated = summary.protectedInventoryRootsTruncated === true; const bytesTruncated = summary.protectedInventoryBytesTruncated === true;
  const lowerBound = (value: unknown, truncated: boolean, fallback = 0) => `${number(value, fallback)}${truncated ? "+" : ""}`;
  const metrics = [["Candidate Paths", number(summary.candidateCount)], ["Findings", count], ["Reviewable", number(summary.reviewableCandidateCount, candidates.length)], ["Protected Roots", lowerBound(summary.protectedInventoryCount, rootsTruncated, protectedEntries.length)], ["Protected Files", lowerBound(summary.protectedInventoryFileCount, bytesTruncated)], ["Protected Directories", lowerBound(summary.protectedInventoryDirectoryCount, bytesTruncated)], ["Protected Bytes", lowerBound(summary.protectedInventoryTotalBytes, bytesTruncated)], ["Manual Review", number(bySeverity.fail)], ["Warn", number(bySeverity.warn)], ["Known Cleanable", number(byCategory["known-cleanable"])], ["Nested Git", number(byCategory["nested-git"])], ["Suspicious", number(byCategory.suspicious)]];
  ...
  const protectedRows = protectedEntries.map((entry) => { const inventory = record(entry); const suffix = inventory.bytesTruncated === true ? "+" : ""; return `<tr><td data-label="Path"><code>${escape(inventory.path)}</code></td><td data-label="Files">${number(inventory.fileCount)}${suffix}</td><td data-label="Directories">${number(inventory.directoryCount)}${suffix}</td><td data-label="Bytes">${number(inventory.bytes)}${suffix}</td><td data-label="Assessment">not-assessed</td><td data-label="Reason">${escape(inventory.reason)}</td></tr>`; }).join("") || empty(6, "No protected inventory recorded.");
  return `<section class="card hygiene ${count > 0 || protectedEntries.length > 0 ? "warning" : "info"}" aria-labelledby="workspace-hygiene-heading"><h2 id="workspace-hygiene-heading">Workspace Hygiene</h2><p class="section-note">Report-only scan of untracked and ignored workspace artifacts. No cleanup actions are performed here.</p>${value.ok === false ? `<p class="hygiene-alert">Scan failed: ${escape(value.reason)}</p>` : ""}<div class="hygiene-metrics">${metrics.map(([label, metric]) => `<article class="hygiene-count"><span>${label}</span><strong>${metric}</strong></article>`).join("")}</div><h3>Protected Inventory</h3><p class="section-note">Protection blocks cleanup authority; it does not prove retained content is useful.</p>${table("Protected workspace inventory", ["Path", "Files", "Directories", "Bytes", "Assessment", "Reason"], protectedRows, "protected-inventory-table")}<h3>Top Findings By Review Level And Category</h3>${table("Top workspace hygiene findings", ["Review Level", "Category", "Path", "Reason"], findingRows, "hygiene-findings")}<h3>Reviewable Candidates</h3><p class="section-note">Plan exact deletes with guardian_delete_paths mode=plan after review. Showing ${number(summary.reviewableShownCount, candidates.length)} of ${number(summary.reviewableCandidateCount, candidates.length)}.</p>${table("Reviewable workspace hygiene candidates", ["Status", "Path", "Reason", "Suggested Command"], candidateRows, "reviewable-table")}</section>`;
}
