...
- `session-finish`: for an active recorded session selected by cwd, by sole dirty target, by `sessionId`, or by `branch`, confirmed apply commits dirty session work only with an explicit `commitMessage`, pushes the branch, creates or reuses the PR, and applies the configured `pullRequestMergeMethod`. The default `merge` mode preserves commit-reachability proof; opt-in `squash` requires an exact approved parent and tree proof. Only then may Guardian remove the stale worktree and local branch. It may fast-forward the local base when safe and may apply token-bound safe redundant cleanup candidates while reporting dirty, protected, unmerged, detached, base-sync, or otherwise unproven leftovers. Admin bypass is never automatic and requires explicit `allowAdminBypass: true`. If the current directory is an existing Guardian-root worktree without an active recorded session, `guardian_done` may first attach a fresh internal recovery session id to that worktree and then finish it through the low-level `guardian_finish` path.
...
Active-session apply must not clean up its own session until the PR merge has completed and the configured transition proof succeeds against the freshly fetched remote base ref. Ordinary refresh is explicitly non-pruning so a server-side source-branch deletion cannot remove cleanup ancestry evidence before Guardian performs its exact-head leased deletion or expected-absence reconciliation. Primary-main apply creates a pre-commit safety ref, commits only token-bound dirty paths, pushes normally to the configured remote/base branch, fetches, proves remote reachability, and applies only token-bound safe redundant cleanup while reporting any remaining blockers. The primary-main lane must not force-push, mutate stashes, delete unrelated or unproven branches, merge PRs, or treat old primary/protected session records as ownership.
...
The token-bound remote base OID is a transition ratchet, not merely an ancestry checkpoint. Immediately before PR merge, the remote base must still equal the planned OID. After merge, Guardian accepts only an unchanged base where the approved head is already reachable, a fast-forward exactly to the approved head, an exact two-parent merge whose parents are the planned base and approved head in that order, or, only when the token-bound method is `squash`, a new commit whose sole parent is the planned base and whose tree exactly equals the approved candidate tree. An arbitrary descendant, octopus merge, reversed-parent merge, unrelated advance, method mismatch, or unchanged base without the approved head blocks cleanup and preserves the session for recovery.

Done-all must reuse each planned child token and authorize it against the current accepted base cursor. It fetches the configured remote directly rather than deriving a remote name from `baseRef`, so valid remote names containing `/` retain their exact identity. The cursor advances only after the child succeeds and the same configured-method transition classifier accepts its observed remote base. Final postflight requires that observed landed commit, not a rewritten pre-squash source commit, to remain on the final base. A failed child may be isolated so later children continue only when the remote base is proven unchanged; any failed child that moved the base, or whose base transition cannot be re-observed, blocks the batch before later children run.
...
Local base divergence requires a separate per-call `allowBaseBranchRealign: true` and an exact `expectedBaseHead` OID. This opt-in applies only when the primary worktree is already on the configured base branch. Guardian rejects missing or stale approval before clearing primary dirt, preserves the old local base in a create-only safety ref, freshly verifies that the remote base is an ancestor of the captured session commit, and realigns locally with non-overwriting `reset --keep` to that remote OID before merging the captured commit. Publication remains a normal non-force push. Registered nested worktrees are excluded from primary dirt regardless of their directory name; incoming writes overlapping those worktrees or ignored untracked paths block. Preserve-reset snapshots stage tracked paths separately from new paths using literal pathspecs, refuse directory/gitlink or ignored-untracked capture, and clean only exact snapshotted paths. Snapshotted dirt remains in its reported recovery ref; finish does not automatically restore it over the merged tree.
...
The archive-backed exception requires one absolute external `archivePath` and its exact lowercase `archiveSha256`. Guardian resolves a pinned system tar executable independently of caller `PATH`, supplies only a fixed minimal tar environment, extracts only the exact current untracked and consented ignored member paths into an isolated temporary directory, compares full POSIX permission bits, size, and SHA-256 or symlink target, and token-binds the normalized archive identity and complete proof. Duplicate members, hardlinks, special files, and ambiguous inventories block. Apply copies the archive through a no-follow file handle, repeats the proof from that private copy after creating the normal worktree safety ref, cooperatively atomically quarantines only those exact paths, verifies the renamed inode, no residue, and no external archive drift, and only then destroys the quarantine before non-force worktree removal. Tracked, staged, renamed, copied, conflicted, deleted, submodule, and type-changed paths retain their existing blockers or redundant-dirty proof lane. Missing members, unsupported types, digest drift, archive replacement, or target drift block and restore quarantined paths when restoration remains unambiguous; a final same-account substitution that changes inode identity is retained in the reported quarantine rather than destroyed.
...
A configured `protectedPaths` untracked or ignored regular-file target may proceed only when `archivePath` and `archiveSha256` prove that every exact selected path is recoverable from a direct external archive. The archive proof is part of the confirm token and is recomputed during apply from a private no-follow copy before cooperative atomic quarantine. This exception never applies to `.git`, tracked source, dependency roots, worktree roots, symlink roots, directories, paths outside the repository, or overlapping selections.
...
`goal.hygieneCompletion` separates cleanup authorization from desired-state completion. Its default, `no-unprotected-residue`, requires zero unprotected findings, zero unacknowledged `reviewableCandidates`, and complete reviewable and filesystem-only inventory coverage before Guardian may delegate commit or push. For a recorded session, Guardian stores the immutable start commit and compares it with the current index; paths added after that baseline remain reviewable after staging or committing on the session branch.

Explicit `authorized-cleanup` preserves legacy permissive completion, while `no-unprotected-findings` gates findings but not reviewables.
...
This setting never broadens deletion. In every completion mode, `guardian_goal` auto-deletes only token-bound `known-cleanable` findings and filesystem-verified empty directories. Fingerprint-bound apply rechecks directory contents, and empty ancestors are removed only while they remain empty. Residual `nested-git` and `suspicious` findings require direct explicit review and are never auto-authorized. A dirty nested repository requires direct `allowDirtyNestedGit` for its own hygiene plan and apply. Configured `protectedPaths` deny deletion and are excluded from strict completion, while their bounded inventory remains visible as `not-assessed`; protection is not evidence that retained content is useful and does not acknowledge a tracked addition. `reviewableCandidates` remain inventory rather than hygiene targets; only `no-unprotected-residue` makes their unresolved presence a completion failure. Resolution remains explicit through one-shot `intentionalPaths`, retention-policy review, or exact `guardian_delete_paths` planning with `allowTracked=true` for a tracked addition.

Goal plans report `complete: null`. A strict plan with safe authorized work and residual findings, reviewables, or incomplete coverage can be actionable with `status: "planned-partial"`. Confirmed apply performs safe hygiene and eligible clean-completion quarantine, then rescans before delivery. A remaining strict postcondition produces a blocked completion child, `ok: false`, `complete: false`, and `status: "partial"`; no commit or push is invoked. Exact repo-relative `intentionalPaths` may acknowledge current regular newly added files for one plan/apply pair. The normalized list is token-bound, never persisted, accepts untracked or tracked-added regular files, rejects baseline tracked files, directories, symlinks, missing paths, globs, and traversal, and never grants deletion authority. The selected baseline commit and source are returned in the machine result and bound into the goal confirmation token.
