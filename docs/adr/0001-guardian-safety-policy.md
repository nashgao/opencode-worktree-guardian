...
- `done-all`: from a clean primary base branch with active Guardian feature sessions, bare `guardian_done` enumerates every active feature session, lands or already-landed-cleans each clean finishable session after confirmation, skips dirty/protected sessions with exact next actions, fast-forwards the local base worktree only when safe, and applies the token-bound redundant cleanup sweep captured by the same clean no-blocker plan before finishing sessions. `all=true` remains accepted but is not required for this normal clean-primary path.
