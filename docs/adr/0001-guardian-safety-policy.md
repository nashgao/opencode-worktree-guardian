...
The archive-backed exception requires one absolute external `archivePath` and its exact lowercase `archiveSha256`. Guardian extracts only the exact current untracked and consented ignored member paths into an isolated temporary directory, compares regular-file mode, size, and SHA-256 or symlink target, and token-binds the normalized archive identity and complete proof. Apply repeats the proof after creating the normal worktree safety ref, removes only those exact paths, blocks on remaining or newly introduced residue, and then uses non-force worktree removal. The archive must remain a direct regular file outside the target worktree; missing members, unsupported types, digest drift, archive replacement, or target drift block.
...
A configured `protectedPaths` regular-file target may proceed only when `archivePath` and `archiveSha256` prove that every exact selected path is recoverable from a direct external archive. The archive proof is part of the confirm token and is recomputed during apply. This exception never applies to `.git`, dependency roots, worktree roots, symlink roots, directories, paths outside the repository, or overlapping selections.
