...
- `guardian_hygiene` scan output now reports the weight of each reviewable candidate. Every `reviewableCandidates` entry carries `fileCount`, the number of scanned paths that collapsed into it, and `summary.reviewableTotalFileCount` sums that across every candidate rather than only the displayed ones, so the total stays accurate no matter how far the list is truncated.
...
- Reviewable hygiene candidates are ordered by descending `fileCount` instead of by path name. The display limit previously retained the alphabetically first entries, which reliably surfaced dot-directories and pushed the largest untracked or ignored trees into the omitted remainder; the visible rows now show the heaviest candidates. The truncation note states that ordering and points at `includeAllReviewableCandidates=true` for the full list.
