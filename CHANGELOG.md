...
- Hygiene reviewable-candidate records now expose `fileCount`, and the summary reports `reviewableTotalFileCount` across the complete candidate set even when the displayed rows are truncated.
...
- Hygiene reviewable candidates are ordered by descending `fileCount`, so truncated output presents the largest cleanup decisions first instead of alphabetically.
- A clean `guardian_status` headline is explicitly scoped to Guardian risk signals and is not presented as a repository-cleanliness claim.
...
- `guardian_status` no longer reports a repository as clean when the hygiene scan found unreviewed paths or failed outright. Reviewable paths and failed scans now emit warn signals; a failure is ordered ahead of findings so unrelated dirt cannot mask unknown cleanliness.
