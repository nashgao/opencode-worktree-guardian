...
- `guardian_goal`: plan or apply the repo-local desired end state from config `goal`. The native tool remains plan-first when `mode` is omitted. Command wrappers should run the plan, inspect child `guardian_hygiene` and `guardian_done` steps, then continue with confirmed apply when the user invoked the goal workflow. The default goal applies safe token-bound `known-cleanable` hygiene cleanup before `guardian_done`, so generated cache residue is not committed, then lets `guardian_done` commit dirty implementation work with an explicit `commitMessage`, land/push to the configured base, sync local base, and clean Guardian-owned stale worktrees/branches through existing Guardian gates. Plans report `complete: null`; strict, actionable plans may be `planned-partial`. Apply must inspect both `complete` and `hygienePostcondition`, not treat `ok` as desired-state completion. With `hygieneCompletion: "no-unprotected-findings"`, a successful apply may return `ok: true`, `complete: false`, and `status: "partial"` after its post-apply rescan. `authorized-cleanup` preserves legacy behavior, so residual findings remain visible but do not prevent completion. Neither mode broadens deletion: residual nested Git and suspicious findings need direct explicit review, and protected paths and reviewable inventory are excluded. `guardian_goal` does not force unmerged abandonment, mutate stashes, raw-clean files, delete unproven branches, or bypass blockers.
...
- Use `guardian_goal` when the user wants the configured desired repo state in one command. It plans first, applies safe token-bound `known-cleanable` hygiene cleanup before `guardian_done`, and then delegates commit, land, push, local base sync, and Guardian-owned worktree/branch cleanup to existing Guardian gates. It still requires explicit confirmation and an explicit `commitMessage` when dirty implementation work must be committed. Inspect `complete` and `hygienePostcondition` after apply. `ok` only confirms authorized work ran. In strict `no-unprotected-findings` mode, a successful apply can remain `partial` with `complete: false` after the post-apply hygiene rescan.
