...
- Use `guardian_goal` when the user wants the configured desired repo state in one command. It plans first, applies safe known-cleanable hygiene cleanup before `guardian_done`, and then delegates commit, land, push, local base sync, and Guardian-owned worktree/branch cleanup to existing Guardian gates. It still requires explicit confirmation and an explicit `commitMessage` when dirty implementation work must be committed.
