...
Guardian hooks do not move the OpenCode host process cwd. Instead, before safe shell/git tools run, Guardian rewrites the tool execution directory to the recorded worktree and then re-applies the destructive-command guards. Direct file mutation tool paths under the primary repo are also rewritten into the recorded worktree when the session owns one, or blocked if the recorded worktree cannot be validated. Missing, unresolvable, or unrecorded worktrees fail closed. Raw cleanup/reset/stash/force-push/worktree-removal commands remain blocked.

Finish normal Guardian work through `guardian_done`, not manual protected-branch push or merge commands. Use `guardian_finish` only for explicit low-level session finishing. Finish always creates a safety ref before risky operations and reports preflight facts/blockers for automation. Dirty worktrees block finish unless every dirty path matches explicit `allowDirtyPaths` config. Allowed dirty paths are reported as `allowedDirtyFiles` and left untouched; any non-matching dirty path remains a blocker. `create-pr` pushes the branch and suggests a PR command; it does not create a PR natively. `merge-to-base` requires explicit approval, and cleanup only runs when `autoCleanup` or `allowCleanup` is set and ancestry is proven.

Use `guardian_done` when implementation is complete and you want Guardian to choose the safe next lane. It is the user-facing done workflow: session-owned work still finishes through `guardian_finish`; clean primary-base cleanup still runs through `guardian_finish_workflow`; dirty protected primary-base work can be published only with an explicit `commitMessage`, explicit confirmation, and the matching internally token-bound dirty snapshot. Apply creates a pre-commit safety ref, commits only the token-bound dirty paths, pushes normally, fetches, proves remote reachability, and returns a fresh cleanup plan for separate confirmation. In the plugin flow, a blank token or confirmation placeholder is treated as absent and the cached internal token is used only when session, repo, options, and dirty snapshot still match the plan. It does not treat old primary/protected session records as ownership, silently apply cleanup after publishing, merge PRs, delete remote branches, force-push, or mutate stashes.
...
Use `guardian_delete_paths` when the user intentionally wants to delete exact files or directories that are not Guardian hygiene findings, including source files. The plan reports each selected path's repo-relative path, absolute path, kind, tracked/ignored/untracked status, tracked contents, and blockers. Apply is still token-bound and re-runs the same fingerprinted preflight immediately before deletion. It blocks paths outside the repo, the repo root, `.git`, `.opencode`, dependency roots such as `node_modules` and `vendor`, configured or registered Guardian worktree roots, the current worktree root, missing paths, symlink roots, overlapping selections, tracked contents without `allowTracked: true`, and directories without `allowRecursive: true`. The tool deletes files with internal Node `fs` APIs and does not stage changes; tracked deletions remain visible in `git status` for review and commit.
