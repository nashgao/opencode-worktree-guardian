...
- Guardian audits command-interception blocks by default. Use native Guardian tools when you want safety refs, plan/apply confirmation, and recovery evidence; set `commandInterceptionMode: "strict"` only in repos that intentionally want hard blocking for hard reset, forced clean, raw worktree removal/prune, raw branch deletion, stash mutation, force push, protected-branch bypasses, broad filesystem deletion, direct file routing failures, or session/worktree integrity mismatches.
...
6. Ask OpenCode to run a known destructive reset command; confirm Guardian logs it with `auditOnly: true`. Repeat with `commandInterceptionMode: "strict"` in the disposable repo config to confirm hard blocking still works.
