...
- For every transition that relies on a base-branch proof, Guardian resolves the authoritative effective base and revalidates immediately before its own handoff. It blocks stale, divergent, or unverifiable state at that observation. The effective authority is normally the configured remote/base, but cleanup-only flows may use a trusted tracked upstream. `guardian_status` reports base distance from locally cached Git refs and does not explicitly refresh refs for that report, so status output cannot prove remote-base freshness.
