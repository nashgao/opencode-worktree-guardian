...
test("package smoke run helper suppresses inherited coverage env", async () => {
  const result = await run(process.execPath, ["-e", "console.log(JSON.stringify({ coverage: process.env.NODE_V8_COVERAGE, marker: process.env.OPENCODE_WORKTREE_GUARDIAN_COVERAGE_RUN, capability: process.env.OPENCODE_WORKTREE_GUARDIAN_COVERAGE_CAPABILITY, compile: process.env.NODE_COMPILE_CACHE }))"], {
    coverage: "suppress",
    env: {
      NODE_V8_COVERAGE: "parent-coverage",
      OPENCODE_WORKTREE_GUARDIAN_COVERAGE_RUN: "parent-marker",
      OPENCODE_WORKTREE_GUARDIAN_COVERAGE_CAPABILITY: "parent-capability",
      NODE_COMPILE_CACHE: "parent-compile-cache",
    },
  });

  assert.deepEqual(JSON.parse(result.stdout), { coverage: "", marker: "", capability: "", compile: "" });
});
