...
test("guardian_finish_workflow dirty primary candidate scan includes read-only candidate inventory", async (t) => {
  ...
  const staleApply = record(await guardianFinishWorkflow({ repoRoot: repo, cwd: repo, mode: "apply", confirmToken: "0".repeat(64) }));
  ...
  const apply = record(await guardianFinishWorkflow({ repoRoot: repo, cwd: repo, mode: "apply", confirmToken: plan.confirmToken }));
