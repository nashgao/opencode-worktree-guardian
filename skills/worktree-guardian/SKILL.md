...
- Use `guardian_done` for normal implementation completion. From a clean primary base branch, bare `guardian_done` plans the repo-wide done-all lane; after confirmation it lands finishable sessions, syncs local base when safe, and reports redundant cleanup candidates for separate confirmation. Prefer it over raw protected-branch push/merge commands or low-level finish tools unless the user explicitly asks for those tools.
