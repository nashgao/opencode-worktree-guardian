...
- `goal.hygieneCompletion`: `authorized-cleanup` by default, preserving legacy completion semantics. `no-unprotected-findings` rejects residual unprotected findings. `no-unprotected-residue` additionally rejects unresolved `reviewableCandidates` or incomplete inventory coverage. No mode broadens deletion authority: every mode auto-deletes only token-bound `known-cleanable` findings. Residual `nested-git`, suspicious findings, and reviewable cleanup require direct explicit review. Configured `protectedPaths` deny deletion and remain excluded from strict completion, but Guardian reports their bounded inventory as `not-assessed`; protection does not prove retained content is useful. Apply rescans before reporting completion.
