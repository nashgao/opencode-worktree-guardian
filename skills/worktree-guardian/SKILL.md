...
- Use `guardian_goal` when the user wants the configured desired repo state in one command. It reads config `goal`, runs safe token-bound `known-cleanable` hygiene cleanup before `guardian_done`, then delegates commit/land/push/worktree/branch cleanup to existing Guardian gates. Run `mode: "plan"` first and inspect `complete` and `hygienePostcondition`; strict actionable plans can be `planned-partial` with `complete: null`. Apply with `confirm: true` only when the user invoked the goal workflow and the plan is safe; respect `freshPlanRequired` by obtaining a new plan. After apply, never equate `ok` with completion. In strict `no-unprotected-findings` mode, `ok: true`, `complete: false`, and `status: "partial"` means Guardian performed authorized work but the post-apply rescan found residual unprotected findings.
