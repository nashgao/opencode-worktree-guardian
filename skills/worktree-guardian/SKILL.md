...
- Use `guardian_goal` when the user wants the configured desired repo state in one command. The strict default targets the selected session worktree, applies safe token-bound `known-cleanable` and verified empty-directory cleanup, and rescans before delegating commit/land/push/worktree/branch cleanup. Run `mode: "plan"` first and inspect `complete`, `hygienePostcondition`, and exact reviewables. Pass legitimate current regular untracked files through exact `intentionalPaths` in both plan and apply; this one-shot list is token-bound and never becomes persistent policy or deletion authority. Strict residue blocks the completion child before commit/push. Explicit legacy completion modes remain available.
