...
- Use `guardian_goal` when the user wants the configured desired repo state in one command. It reads config `goal`, runs safe token-bound `known-cleanable` hygiene cleanup before `guardian_done`, then delegates commit/land/push/worktree/branch cleanup to existing Guardian gates. Run `mode: "plan"` first and inspect `complete` and `hygienePostcondition`; strict actionable plans can be `planned-partial` with `complete: null`. Apply with `confirm: true` only when the user invoked the goal workflow and the plan is safe; respect `freshPlanRequired` by obtaining a new plan. When exact remote refs on the resolved effective remote must be retained for one completion call, pass their exact names through `allowedRemoteBranches` in both plan and apply. This per-call option is normalized and token-bound, leaves same-named local branches independently cleanup-eligible, and does not affect unscanned secondary remotes or persist retention policy. After apply, never equate `ok` with completion. In strict `no-unprotected-findings` mode, `ok: true`, `complete: false`, and `status: "partial"` means Guardian performed authorized work but the post-apply rescan found residual unprotected findings.
...
- `goal.hygieneCompletion`: `authorized-cleanup` by default, preserving legacy completion semantics. `no-unprotected-findings` makes no residual unprotected hygiene finding a completion postcondition, without broadening deletion authority. Both modes auto-delete only token-bound `known-cleanable` findings. Residual `nested-git` and `suspicious` findings require direct explicit review; dirty nested Git additionally requires direct `allowDirtyNestedGit`. Configured `protectedPaths` are intentional retention and excluded, while `reviewableCandidates` are inventory rather than strict failures. Apply rescans before reporting completion.
